...
- singular permission is a cleaner way to let different teams do different patterns
- context as an object would be helpful to not force people to decode
- remove expiry as top-level type, let teams decide for their permission type
- security concerns with 7679/prepareCalls model with user account as userOp.sender
  - key signers are likely vulnerable to our audit finding because it encourages using the account as userOp.sender
- rename `accountMeta` to `initCode`
