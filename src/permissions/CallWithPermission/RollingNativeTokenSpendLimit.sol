...
abstract contract RollingNativeTokenSpendLimit is UserOperationUtils {
    ...
    ///
    /// @dev last mapping key must be account address for 4337 slot access.
    mapping(bytes32 permissionHash => mapping(uint256 spendIndex => mapping(address account => Spend spend))) private _permissionSpend;
    ...
    function assertSpend(uint256 spendValue, bytes32 permissionHash, uint256 spendLimit, uint256 spendPeriod) external {
        ...
        if (spendValue + rollingSpend > spendLimit) revert ExceededSpendingLimit();
        // add spend to state
    ...
    ///
    /// @dev Does not protect against re-entrancy where an external call could trigger an authorized call back to the account to spend more ETH.
    /// @dev Does not measure or account for ETH spent via gas if a paymaster is not used.
    function _validateAssertSpendCall(uint256 spendValue, bytes32 permissionHash, uint256 spendLimit, uint256 spendPeriod, Call memory call) internal view {
        bytes memory assertSpendData = abi.encodeWithSelector(RollingNativeTokenSpendLimit.assertSpend.selector, spendValue, permissionHash, spendLimit, spendPeriod);
    ...
    function _calculateRollingSpend(address account, bytes32 permissionHash, uint256 spendPeriod) internal view returns (uint256 rollingSpend) {
